...
##
#  Module defining a special Date Index - 
#  - Normalized value has granularity of one *day*.
#  - Objects which don't have an indexable attribute value are *indexed* with a default value
#  - Objects which have 'None' as indexed value are *omitted*
##
...
from Products.PluginIndexes.DateIndex.DateIndex import LocalTimezone

_marker = []
_default = DateTime(1000, 0) #default value when an object does not have the appropriate attribute
Local = LocalTimezone()


class CedesDateIndex(UnIndex, PropertyManager):
    ...
    manage = manage_main = DTMLFile( 'dtml/manageCedesDateIndex', globals() )
    ...
    def index_object( self, documentId, obj, threshold=None ):
        """index an object, normalizing the indexed value to an integer
           ...
           o Normalized value has granularity of one *day*.
           
           o Objects which don't have an indexable attribute value are *indexed* with a default value
             by design.
        ...
        try:
            ...
            #KIM We only includes the ressources for the cedes project (optimization)
            if obj.portal_type in ('ArticleGratuit', 'ArticlePayant', 'SiteInternet', 'Statistiques', 'Audio', 'Video', 'Cederom', 'Bibliographie', 'SequenceApprentissage'):
              ConvertedDate = self._convert( value=_default )
            else:
    ...
    def _apply_index( self, request, cid='', type=type ):
        ...
        if opr=="range":   # range search
            ...
            #adds the default objects to the reult set
                
            set = index.get(self._convert( value=_default, default=_marker ), None)
            if set is not None:
            ...
            #appends the default date
            keys.append(self._convert( value=_default, default=_marker ))
            for key in keys:
    ...
    def _convert( self, value, default=None ):
        """Convert Date/Time value to a Date Integer (Granularity is One day)"""
...
manage_addCedesDateIndexForm = DTMLFile( 'dtml/addCedesDateIndex', globals() )

def manage_addCedesDateIndex( self, id, REQUEST=None, RESPONSE=None, URL3=None):
    """Add a Date index"""
    return self.manage_addIndex(id, 'CedesDateIndex', extra=None, \
